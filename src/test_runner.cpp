...
void TestRunner::run_histogram_tests() {
    std::vector<int> test_sizes = {10000000, 50000000, 100000000}; 
    const int NUM_BINS = 256; 
    const int MIN_VAL = 0;
    const int MAX_VAL = NUM_BINS - 1;
    ...
    for (int N : test_sizes) {
        ...
        std::vector<int> bins_ref(NUM_BINS, 0);
        std::vector<int> bins_simd(NUM_BINS, 0);
        std::vector<int> bins_omp(NUM_BINS, 0);
        std::vector<int> bins_omp_simd(NUM_BINS, 0);

  
        double t_serial = measure_time([&](){ 
            histogram_serial(datos.data(), N, bins_ref.data(), NUM_BINS, MIN_VAL, MAX_VAL); 
        });
        ...
        double t_simd = measure_time([&](){ 
            histogram_simd(datos.data(), N, bins_simd.data(), NUM_BINS, MIN_VAL, MAX_VAL); 
        });
        ...
        double t_omp = measure_time([&](){ 
            histogram_omp(datos.data(), N, bins_omp.data(), NUM_BINS, MIN_VAL, MAX_VAL); 
        });
        ...
        double t_omp_simd = measure_time([&](){ 
            histogram_omp_simd(datos.data(), N, bins_omp_simd.data(), NUM_BINS, MIN_VAL, MAX_VAL); 
        });
